...
> **"the loop-foreman should be part of the companions list for people to opt-out of."**

> **"I like the idea of breaking up the list between Evaluators and Helpers."**

*— Added 2026-06-25, source: user directive (ratified after a 3-agent panel — regression-hunter / premortem-analyst / skeptic — mapped the conflict surface; the user ratified both the addition and the two-group split across two messages in the same session). Adds `loop-foreman` as a FIFTH companion to the Step 0.3 Companion-Skill Selection Gate's offerable table, and reshapes that single gate into TWO grouped multi-selects — **Evaluators** (`text-eval`, `code-evaluator`) and **Helpers** (`route`, `awareness-ledger`, `loop-foreman`) — rendered as two question objects inside ONE `AskUserQuestion` call. The split is forced by a tool limit, not a policy one: `AskUserQuestion` caps options at four per question object and the gate already held four companions; grouping keeps each object ≤4 while remaining ONE prompt screen. Because two question objects in one call is one question slot (the Lane→Model picker precedent — two model questions, one call, one slot), the **sanctioned-question count stays FIVE** — this directive adds ZERO new audit questions; it only widens an existing one. It widens the 2026-06-24 Companion-Skill directive's "the evaluator/helper companions audit force-installs" definition (annotation clause 2) to "force-installs OR offers", superseding newest-wins ONLY that scoping phrase so an offer-only companion may appear in the gate; every other hardening of that directive — inverted widget, provenance guard, "Always defer to strip", headless suppression, the destructive-work-DEFERS floor — stands verbatim and untouched. `loop-foreman` is the FIRST companion that is **offered, never auto-fired** (2026-06-25 loop-foreman-design decision), so it carries an `install-on-absence: no` attribute the other four lack: unchecked + absent resolves to a NO-OP (never an install authorization), and the headless / marker-absent / bootstrap install-on-absence default set explicitly EXCLUDES it — preserving offer-don't-seize, the most-precedented failure the loop-foreman design guarded against. Its presence test is name-based (`.claude/skills/loop-foreman/SKILL.md` exists — matching `route`/`awareness-ledger`, never a loose signal that could collide with the host's `/loop` or `/schedule`); checked + present + skill-builder-scaffolded DEFERS `/skill-builder strip loop-foreman --execute --confirm-breaking` (it registers two agents and is route-wired, so strip is BREAKING), never an AUTO delete. A `loop-foreman=on|off` key joins the `companion-skills` marker, where `=on` means only "keep if present, never auto-install"; a MISSING key on a pre-2026-06-25 project reads as neutral/leave-as-is, never install. `strip`'s known-companion disconnection sweep gains `loop-foreman` and its two agents. The split-answer resolution, install-on-absence attribute, marker divergence, and presence test live in references/procedures/audit.md § Step 0.3 + § Step 6, references/model-lanes.md § Companion Skills, and references/procedures/strip.md pattern 12, never inside this immutable block.)*
<!-- /origin -->
...
<!-- Amending directive (2026-06-25, loop-foreman opt-out + Evaluators/Helpers split): "the loop-foreman should be part of the companions list for people to opt-out of." + "I like the idea of breaking up the list between Evaluators and Helpers." Widens the offerable set to "force-installs OR offers"; loop-foreman carries install-on-absence: no; the gate renders as TWO grouped multi-selects in ONE call (count stays FIVE). -->
...
1. SUPPRESSION FIRST. IF the run is headless / non-interactive OR `audit --quick` → STOP this gate silently: render no checkbox, write no marker, REMOVE nothing. Headless install behavior falls back to the persisted `<!-- companion-skills: … -->` marker, or — marker absent — to the pre-2026-06-24 install-on-absence / ensure-exists defaults. The install-on-absence default set is the FOUR install-on-absence companions ONLY (`text-eval`, `code-evaluator`, `route`, `awareness-ledger`); `loop-foreman` (`install-on-absence: no`) is NEVER in it — a suppressed run installs nothing for it. Absence of an answer NEVER means remove.
2. THE OFFERABLE SET is the evaluator/helper companions audit force-installs OR offers (definition widened newest-wins by the 2026-06-25 directive from "force-installs" to "force-installs or offers", so an offer-only companion may appear): `text-eval` (Step 4c-bis), `code-evaluator` (Step 4a-bis), `route` (Step 4g), `awareness-ledger` (Step 4a), and `loop-foreman` (offered via `/route` or `loop-foreman create`, NEVER auto-installed). Each row carries an `install-on-absence` attribute: `yes` for the first four, `no` for `loop-foreman`. NO "(recommended)" label is rendered in this gate (the 2026-06-24 directive's "(recommended)" mechanic for `route`/`awareness-ledger` is SUPERSEDED newest-wins by the 2026-06-24 inversion follow-up — under the inverted widget of clause 3, a "(recommended)" tag next to a remove-checkbox reads as "recommended to remove"; the user ratified dropping it). `route` and `awareness-ledger` remain the install-on-absence defaults, so the empty default keeps/installs them. The immutable directive text is unchanged — only the set-scope phrase and rendering mechanic are superseded. The set is an enumerated table in audit.md § Step 0.3 so future companions extend it by adding a row, never by editing this block.
3. NO PRE-CHECK POSSIBLE → INVERTED WIDGET, RENDERED AS TWO GROUPS. `AskUserQuestion` has no `selected`/`default` field, so a multi-select CANNOT render pre-checked boxes (every box renders empty). The directive's "checkmarks by present items" intent is honored by making the empty default SAFE, not by a rendered checkmark: the gate INVERTS — an UNCHECKED box keeps/installs the companion, a CHECKED box opts it OUT (remove if present / skip install if absent). The gate renders as TWO multi-select question OBJECTS inside ONE `AskUserQuestion` call — **Evaluators** (`text-eval`, `code-evaluator`) and **Helpers** (`route`, `awareness-ledger`, `loop-foreman`) — because the tool caps options at four per object (2026-06-25 directive). Two objects in one call is still ONE prompt screen and ONE sanctioned question slot (Lane→Model picker precedent), so the FIVE-question count is unchanged. Presence is computed per the table: function-/signal-based for `text-eval` (Step 4c-bis / § Build Scaffolds test, any FUNCTION-performing skill counts under any name), and name-based for the named companions (`code-evaluator`, `route`, `awareness-ledger`, `loop-foreman` → `.claude/skills/<name>/SKILL.md` exists — never a loose signal that could collide with the host's `/loop` or `/schedule`). Presence is surfaced in each row's label/description as PRESENT/ABSENT, NOT as a pre-check. The marker, when it exists, is reconciled against on-disk presence; on-disk wins, the marker records intent. (Inversion ratified 2026-06-24 — tool-capability reconciliation; present companions are retained BY DEFAULT.)
4. PROVENANCE GUARD (removal floor). Removal may target ONLY a skill skill-builder itself scaffolded (scaffold-provenance marker present). A function served by a user's hand-authored skill is detected-as-present (so no duplicate install is offered) but is NEVER eligible for removal: checking it for opt-out writes the `=off` intent and prints an informational note ("served by your own skill `<name>`; skill-builder will not remove a skill it did not create"), never a `strip` row. (`loop-foreman create` scaffolds a skill-builder-provenance skill, so a present scaffolded `loop-foreman` IS removable; a hand-authored loop driver is note-only.)
5. UNCHECKED = KEEP (default) resolution, BRANCHED ON `install-on-absence`: unchecked + present → keep; write `<name>=on`; no action. Unchecked + absent + `install-on-absence: yes` → AUTHORIZE the companion's existing AUTO install task (this gate is now the authorization for Steps 4a / 4a-bis / 4c-bis / 4g appends; they no longer fire unconditionally); write `=on`. Unchecked + absent + `install-on-absence: no` (`loop-foreman`) → NO-OP: authorize NOTHING (it is offered, never auto-fired); write `=on` meaning only "keep if present, never auto-install". Never iterate the table blind — read the attribute before authorizing any install.
6. CHECKED = OPT-OUT resolution: write `<name>=off`. IF absent → suppress the install (this is the opt-out; supersedes newest-wins the 2026-06-12 "absence alone" / "by default, not opt-in" auto-installs for this companion); for `install-on-absence: no` companions there is nothing to suppress (it was never going to install) — `=off` simply records "not wanted". IF present AND skill-builder-scaffolded → DEFER a `/skill-builder strip <name> --execute` row (add `--confirm-breaking` when HARD references exist — always for `loop-foreman`, which registers two agents and is route-wired) to the Deferred Items table. NEVER an AUTO delete — the user ratified "Always defer to strip", and the destructive-work-DEFERS floor (deletion delegated to strip, never reimplemented) is honored, not superseded. The DEFER note states that strip performs the COMPLETE cross-reference disconnection across every other skill plus the `route index`/embed refresh (why this gate fires THIRD — before the audit commits its terminal route tasks).
7. THE ONE WRITE, RESOLVED ACROSS BOTH GROUPS. This gate's only write is the per-project `<!-- companion-skills: text-eval=…,code-evaluator=…,route=…,awareness-ledger=…,loop-foreman=… -->` marker in references/model-lanes.md (the checkbox answer IS the consent, like the Lane→Model picker). Resolve EVERY companion across BOTH question objects before the write and assert coverage — every offerable companion must map to exactly one `on`/`off`; abort the write if any key is unresolved (guards the split against a dropped second-group answer). A MISSING `loop-foreman` key on a pre-2026-06-25 marker reads as neutral/leave-as-is (`install-on-absence: no` → never install), never as an install authorization. Back-compat: a legacy `<!-- creative-scrub-build: off -->` marker reads as `text-eval=off`. It never switches the session model and never asks one to.
8. IF any change proposes auto-deleting on the opt-out selection (a checked box under the inverted widget), auto-installing `loop-foreman` (or any `install-on-absence: no` companion) on absence, exposing a user-authored skill for removal, removing in a headless run, or removing without routing through `strip` → STOP and report the directive conflict.
<!-- END ENFORCEMENT ANNOTATION -->
