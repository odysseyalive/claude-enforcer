...
3. BUILD tier (2026-06-11 second clause — audit builds the missing machinery, additively):
   ...
   - **New-skill scaffold (AUTO only on declared evidence):** when the project has ≥1 skill with a DECLARED creative lane (frontmatter or Skill→Lane table — declared, never inferred) producing content with NO evaluator skill to chain to → scaffold the evaluator from § Build Scaffolds (persona + model gates apply in full). Inferred-only creative classification → DEFER with the exact command.
   - **Atomic-or-absent:** a build that cannot complete its panel/anchor checks writes NOTHING and becomes a DEFER row. Reconciliation on later audits touches only `modifiable: true` machine bytes (code-eval-grade user seam); `creative-scrub: off` frontmatter opts a skill out entirely.
