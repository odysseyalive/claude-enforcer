...
2. THE OFFERABLE SET is exactly the evaluator/helper companions audit force-installs: `text-eval` (Step 4c-bis), `code-evaluator` (Step 4a-bis), `route` (Step 4g), `awareness-ledger` (Step 4a). NO "(recommended)" label is rendered in this gate (the 2026-06-24 directive's "(recommended)" mechanic for `route`/`awareness-ledger` is SUPERSEDED newest-wins by the 2026-06-24 inversion follow-up — under the inverted widget of clause 3, a "(recommended)" tag next to a remove-checkbox reads as "recommended to remove"; the user ratified dropping it). `route` and `awareness-ledger` remain the install-on-absence defaults, so the empty default keeps/installs them. The immutable directive text is unchanged — only this rendering mechanic is superseded. The set is an enumerated table in audit.md § Step 0.3 so future companions extend it by adding a row, never by editing this block.
3. NO PRE-CHECK POSSIBLE → INVERTED WIDGET. `AskUserQuestion` has no `selected`/`default` field, so a multi-select CANNOT render pre-checked boxes (every box renders empty). The directive's "checkmarks by present items" intent is honored by making the empty default SAFE, not by a rendered checkmark: the gate INVERTS — an UNCHECKED box keeps/installs the companion, a CHECKED box opts it OUT (remove if present / skip install if absent). Presence is computed by the SIGNAL-BASED existence test (never name-based — reuse the Step 4c-bis / § Build Scaffolds test): a skill performing the companion's FUNCTION counts as present under any directory name; presence is surfaced in each row's label/description as PRESENT/ABSENT, NOT as a pre-check. The marker, when it exists, is reconciled against on-disk presence; on-disk wins, the marker records intent. (Inversion ratified 2026-06-24 — tool-capability reconciliation; present companions are retained BY DEFAULT.)
...
5. UNCHECKED = KEEP (default) resolution: unchecked + absent → AUTHORIZE the companion's existing AUTO install task (this gate is now the authorization for Steps 4a / 4a-bis / 4c-bis / 4g appends; they no longer fire unconditionally); write `<name>=on`. Unchecked + present → keep; write `=on`; no action.
