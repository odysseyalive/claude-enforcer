...
> **"I'd like to adjust the checkboxes for audit that is meant to install/uninstall custom skills built by this package. I'd like to make this much easier. I'd like to check the boxes of what people want to install, (or update). uninstall can happen manually. This will remove some of the confusion. ... also, claude-fable-5 is not part of the advisor pool for that check area. All model selections should work off the same pool to choose from."**

*— Added 2026-07-17, source: user directive (design vetted by a 3-agent panel — regression-hunter / premortem-analyst / skeptic — before authoring). Reshapes the Step 0.3 Companion-Skill Selection Gate into a NATURAL install-only widget: **a checked box means INSTALL; an unchecked box means nothing happens; the gate never uninstalls** — removal is exclusively the user's own manual `/skill-builder strip <name> --execute`, whenever they choose. Only ABSENT companions render as checkbox options (a present companion has nothing a checkbox could authorize: its updates — code-eval `sync`, forced catalog propagation, the terminal route tasks — already run unconditionally, satisfying the directive's "(or update)" clause; present companions are listed informationally with the exact `strip` command named, per the 2026-06-11 named-command advisory). A group with no absent members is omitted; all four present → the gate is a one-line notice, not a question. The empty default is inert SYMMETRICALLY: an interactive empty submission installs nothing, and the headless/`--quick` marker-absent fallback likewise installs nothing (a user-ordered reversal of the install-on-absence default, so the two modes can never disagree; a persisted `<name>=on` marker still authorizes headless installs). Marker semantics: `on` = installed/authorized; `off` = "do not auto-install while absent," NEVER "remove"; a present-but-not-rendered companion PRESERVES its existing marker value (a prior hand-set `off` is never silently flipped — the INC-2026-07-01 keep-semantics lesson), and a missing key resolves `on`. The "(recommended)" labels on `route` and `awareness-ledger` are RESTORED — the original 2026-06-24 parenthetical wording reads correctly again once a check means install. Supersedes newest-wins ONLY: (a) the 2026-06-24 directive's removal clause ("If an item is unchecked, then audit should remove that skill set from the project") and its "Always defer to strip" DEFER-row mechanic — strip remains the sole deletion path, now user-initiated only, so the destructive-work-DEFERS floor is honored by subtraction; (b) the 2026-06-24 inversion follow-up's inverted-widget and no-"(recommended)" rendering mechanics; (c) the 2026-06-12 install-on-absence default as an unconsented trigger in BOTH interactive-empty and headless-marker-absent states. The 2026-06-25 Evaluators/Helpers split, the one-call/one-slot rendering, the FIVE-question ceiling (now a ceiling the gate may come in under), the headless suppression rule, the marker persistence, and the provenance/no-duplicate guard all stand unchanged. Second clause (advisor pool): every model-choosing question — the two lane questions AND the global advisor question — draws from the ONE shared pool defined at lane-delegation.md § Lane→Model Picker (statics newest-first plus the live-discovered latest; `claude-fable-5` always offered in the advisor question and never dropped); consumer files must reference that pool, never restate a private option list. The gate, marker scheme, and pool mechanics live in references/procedures/audit.md § Step 0.3 and § Step 6, references/model-lanes.md § Companion Skills, and references/lane-delegation.md § Lane→Model Picker + § Global Advisor Model, never inside this immutable block.)*

> **"All deferred actions need to be automatically taken care of without user permission or prompting."**

*— Added 2026-07-22, source: user directive (issued in reaction to audit runs that produce Deferred Items tables requiring new users to manually discover and run follow-up commands — "That's too much to ask, especially for new users of this tool"). Eliminates the DEFER tier from audit's AUTO/DEFER classification: every item that previously deferred to a manual follow-up command now auto-executes in the Step 6 phase under the Step 0 disclaimer consent. Agent panels — never user questions — provide the judgment that deferred items previously pushed to the user: strip redundancy confirmation (with BREAKING detection still firing; agent panel replaces the `--confirm-breaking` user gate), quarantine repair determination, dead-wiring recover-vs-unwire calls, AMBIGUOUS bootstrap candidate resolution (conservative on panel disagreement), directive-protection sidecar generation and hook wiring, code-eval enforcement wiring and refresh, convert ratification decisions, and creative-integrity guard-failure adjudication. The Step 0.2 backup is the recovery mechanism for destructive operations and for git-dependent operations in no-VCS projects (backup taken → proceed; backup declined → proceed with warning). Failed auto-execution tasks land in the Execution Summary as ✗ items with explanations — never as "run this command" deferrals. Supersedes newest-wins ONLY the DEFER tier and "destructive-work-DEFERS floor" of every prior directive that defined them: the 2026-06-06 Audit Autonomy Gate's clause 4 DEFER list, the 2026-06-24 "Always defer to strip" DEFER-row mechanic (strip auto-runs under agent-confirmed redundancy when reconcile confirms a completion-breaking conflict; the companion gate's non-removal stance is unchanged — it authorizes installs only; strip fires from RECONCILE findings, never from the companion gate), the code-eval enforce and directive-protection "deliberate host act" DEFER framing (the audit IS running on the host; the Step 0 disclaimer IS the deliberate consent), and the creative-integrity DEFER tiers for guard-failed and equivalence-uncertain items. Every other hardening stands: immutable blocks remain FLAG-NEVER-TOUCH, builds are strictly additive, agent panels fire as written, the backup-before-mutation sequence is preserved, VCS preflight still warns, headless runs remain scan-only, `restore` is never auto-fired (distinct from strip — overwrites not deletes), standalone command gates keep their own `--execute` requirements when invoked outside audit, and the FIVE sanctioned audit questions are unchanged (no new questions added — agent panels replaced the judgment that DEFERRALS previously pushed to manual follow-up). The execution mechanics live in references/procedures/audit.md § Step 6, never inside this immutable block.)*
<!-- /origin -->
...
4. A minion with all REQUIRES present but multiple valid interpretations returns `AMBIGUOUS: <question>` — the orchestrator owns the resolution. Under `audit` (Audit Autonomy Gate + 2026-07-22 no-deferrals directive): agent panel → conservative alternative → auto-execute — never a mid-run user question, never a deferral. Outside audit, AskUserQuestion remains available per Rule 8. A minion NEVER guesses, and `INCOMPLETE:` still covers missing inputs.
...
3. BUILD tier (2026-06-11 second clause — audit builds the missing machinery, additively):
   ...
   - **Pattern-library gap appends (AUTO — 2026-06-12 second clause):** demonstrably absent mechanisms from the shipped catalog are APPLIED, never deferred: appended into the machine-owned region of scaffold-generated libraries, or — for hand-authored libraries — into ONE machine-owned `origin: skill-builder | modifiable: true` appendix region at the end of the library file. Existing bytes never modified; structural fit verified before the write (no parseable fit → agent panel adjudicates the placement per the 2026-07-22 no-deferrals directive); `.scrub-gaps.acked` is the sole dedup authority; the append fires only when the skill's evaluator grounds against the full library file (else agent panel adjudicates — inert-sidecar lesson). Equivalence-uncertain stays silently skipped. **Forced upgrade on catalog growth (2026-06-12 third clause):** a shipped version anchor newer than what a library was last checked against → the gap comparison RUNS and its AUTO appends APPLY in that same audit (never skipped, never offered); scaffold-generated libraries' machine-owned regions version-sync to the current catalog; the ack sidecar suppresses only slugs whose shipped entry is unchanged — new signs always land.
   - **Atomic-or-absent:** a build that cannot complete its panel/anchor checks writes NOTHING and is reported as a ✗ item in the Execution Summary (2026-07-22 no-deferrals directive — failed tasks are reported, never deferred). Reconciliation on later audits touches only `modifiable: true` machine bytes (code-eval-grade user seam); `creative-scrub: off` frontmatter opts a skill out entirely, and the project-level `<!-- creative-scrub-build: off -->` marker in `model-lanes.md` suppresses the new-skill scaffold.
...
| **Destructive** (deletes files irreversibly) | `strip` | **Display mode** (requires `--execute`; `--confirm-breaking` if dependents exist) *when invoked standalone; under audit, auto-fires with agent-panel confirmation per the 2026-07-22 no-deferrals directive* |
...
3. **Audit calls sub-commands in display mode for the scan, then auto-executes** (2026-06-06 Audit Autonomy Gate + 2026-07-22 no-deferrals directive): after the report and its Execution Plan block, audit runs the full task list under the Step 0 disclaimer consent — there is no DEFER tier; everything auto-executes with agent panels providing judgment where needed. Failed tasks appear in the Execution Summary as ✗ items, never as deferred commands. `audit --review` is the zero-write opt-out.
...
7. **Model-lane reporting + audit disclaimer.** Model-switch prompts are ABOLISHED everywhere in skill-builder (2026-06-06 No-Switch-Prompt directive, superseding the earlier "flag + prompt" mechanic newest-wins): when `audit` finds a skill whose declared lane's preferred model differs from the active session model (see § Directives → Model-Lane Routing Gate and audit.md § Step 4f), it **reports** the mismatch as a one-line advisory and proceeds — never an AskUserQuestion, never an instruction to run `/model`. `--model-prompt` is retired; `--no-model-prompt` is a harmless no-op. **Disclaimer (every audit):** before any audit work begins, the user must accept the audit disclaimer (designed for Opus 4.7+; skills created are backwards compatible with earlier models; back up CLAUDE.md and .claude first) — see § Directives → Audit Disclaimer Gate and audit.md § Step 0. Headless runs require a prior interactive acceptance marker. **Onboarding:** when a project has no lanes configured yet, a full interactive `audit` offers a one-time setup question (Set it up now / Not now / Never ask in this project), tracked per project via the `model-lane-setup` marker in `model-lanes.md` — this configures the mapping; it never requests a switch. Bootstrap audits on fresh projects included (2026-06-07 fix — bootstrap mode runs Step 4f after its auto-execution phase; see audit.md § Step 2.5). See audit.md § Step 4f-setup and model-lanes.md § Setup State. **Picker (every full audit):** once configured, every full interactive audit re-confirms the Lane→Model mapping via a batched picker — options exactly `claude-fable-5`, `claude-opus-4-8`, `claude-opus-4-6` (newest-first), and the latest released model by official ID (discovered live, never fabricated), rendered within AskUserQuestion's four-option ceiling (oldest static dropped on overflow; see lane-delegation.md § Lane→Model Picker — the ONE shared pool every model question draws from, lane and advisor alike), current values pre-selected. A confirmed remap fans out to every generated `lane-pinned:` agent's `model:` frontmatter (Fleet Rewrite). The picker fires UP FRONT in the question cluster (audit.md § Step 0.4), not mid-scan — it was being skipped when buried at the old "Step 4f step 2-bis" location. See audit.md § Step 0.4 and references/lane-delegation.md. **Advisor (2026-07-08):** the picker call carries a third question object — the global advisor model (Claude Code's advisor tool: a second, at-least-as-capable model consulted at decision points). Options are FULL OFFICIAL MODEL IDs (the Lane→Model Picker's static IDs plus the live-discovered latest, never the `fable`/`opus`/`sonnet` aliases) plus "No advisor", filtered by the at-least-as-capable pairing rule against the detected main model (Claude Code itself silently detaches invalid pairings). ONE global advisor, never per-lane; lane-pinned agents inherit it with per-agent pairing re-validation. Configuring it writes the `advisor-setup` marker in `model-lanes.md` plus a host-local `advisorModel` (the full model ID) in `.claude/settings.local.json` and names `/advisor <full-model-id>` for immediate effect — a configuration carve-out like the picker itself, never a switch prompt. Spec: lane-delegation.md § Global Advisor Model. **Excursions never advise:** cross-lane mid-workflow steps delegate to lane-pinned agents instead (§ Directives → Bespoke Excursion-Agent Gate).
8. **Decision handoffs use AskUserQuestion — never a free-text "should I?".** Every point where a command ends its turn to let the user decide *whether to proceed, what to change, or which direction to take* MUST be presented via **AskUserQuestion** (a clickable menu), not as end-of-turn prose like "Would you like me to proceed?". This holds for **every** command, display and execute alike, and generalizes Rules 3, 6, and 7 into one invariant. **Audit carve-out (2026-06-06 Audit Autonomy Gate, count raised to four by the 2026-06-24 Companion-Skill Selection Gate directive and to five by the 2026-06-24 Backup Offer directive):** under `audit` the only permitted questions are the Step 0 disclaimer, the Step 0.2 backup offer (fired second), the Step 0.3 Companion-Skill Selection Gate (up to two grouped multi-selects — Evaluators / Helpers, absent companions only, check-to-install per the 2026-07-17 directive — in one AskUserQuestion call, one question slot; fired third; renders as an informational line instead when every companion is installed), the one-time 4f-setup onboarding, and the every-audit Lane→Model picker — audit makes no other decision handoffs; it does ALL the work, with agent panels resolving judgment calls and failed tasks reported in the Execution Summary (informational prose, not a handoff). **Over-fire guard:** if you cannot name at least two concrete options, it is not a decision handoff — ask in prose (open-ended freeform like "paste the error text" stays prose; in-turn status, progress narration, bare acknowledgements, and audit's terminal Execution Summary never prompt). **Option floor:** every decision menu offers at least the recommended path, one genuine alternative, and an explicit **"Skip / not now"** so the user is never trapped into authorizing an action (AskUserQuestion's auto-appended "Other" preserves freeform on top of these). Style on the `update` CHECKPOINT (references/procedures/update.md).
<!-- /origin -->
...
- [references/lane-delegation.md](references/lane-delegation.md) — Lane-pinned excursion delegation: per-step classification, NON-DELEGABLE list, Context Contract, agent template, Delegation Map block, Lane→Model picker + global advisor question + fleet rewrite (read by agents § Step 4d, route § Step 9, audit § Step 0.4 picker + § Step 4f scan)
