...
1. FOUR QUESTIONS ONLY (the count was raised from three to four newest-wins by the 2026-06-24 Companion-Skill Selection Gate directive; this annotation's source-directive text and clause numbering are otherwise unchanged). An audit run may ask the user exactly: (a) the Step 0 disclaimer (Accept and proceed / Cancel audit), (b) the Step 0.3 Companion-Skill Selection Gate (the multi-select checkbox, fired SECOND right after the disclaimer — suppressed in headless / non-interactive / `audit --quick`), (c) the one-time 4f-setup lane onboarding, and (d) the Lane→Model picker — (a), (b), and (d) asked EVERY full interactive audit run, regardless, per the user's verbatim instruction; (c) is one-time. NO other AskUserQuestion, y/n offer, execution menu, or end-of-turn decision question may be emitted anywhere in the audit path, including bootstrap mode, quick mode, chained sub-commands, and the post-action chain when its parent is audit. The companion gate's answer is a SECOND consent instrument alongside the disclaimer: it authorizes the install half (AUTO) and feeds the removal half into the DEFER `strip` tier (clause 4) — it never auto-deletes.
...
CHECKPOINT — Companion-Skill Selection Gate (fires SECOND in every full interactive `audit`, immediately after the Step 0 disclaimer is accepted and BEFORE Step 0.5; see audit.md § Step 0.3):
1. SUPPRESSION FIRST. IF the run is headless / non-interactive OR `audit --quick` → STOP this gate silently: render no checkbox, write no marker, REMOVE nothing. Headless install behavior falls back to the persisted `<!-- companion-skills: … -->` marker, or — marker absent — to the pre-2026-06-24 install-on-absence / ensure-exists defaults. Absence of an answer NEVER means remove.
2. THE OFFERABLE SET is exactly the evaluator/helper companions audit force-installs: `text-eval` (Step 4c-bis), `code-evaluator` (Step 4a-bis), `route` (Step 4g), `awareness-ledger` (Step 4a). `route` and `awareness-ledger` MUST be labeled `(recommended)` in their checkbox row (user directive) and pre-checked. The set is an enumerated table in audit.md § Step 0.3 so future companions extend it by adding a row, never by editing this block.
3. PRE-CHECK = PRESENT, by the SIGNAL-BASED existence test (never name-based — reuse the Step 4c-bis / § Build Scaffolds test): a skill performing the companion's FUNCTION counts as present under any directory name. A box is pre-checked iff the function is present. The marker, when it exists, is reconciled against on-disk presence; on-disk wins for the pre-check, the marker records intent.
4. PROVENANCE GUARD (removal floor). Removal may target ONLY a skill skill-builder itself scaffolded (scaffold-provenance marker present). A function served by a user's hand-authored skill is detected-as-present (so no duplicate install is offered) but is NEVER eligible for removal: unchecking it writes the `=off` intent and prints an informational note ("served by your own skill `<name>`; skill-builder will not remove a skill it did not create"), never a `strip` row.
5. CHECKED resolution: checked + absent → AUTHORIZE the companion's existing AUTO install task (this gate is now the authorization for Steps 4a / 4a-bis / 4c-bis / 4g appends; they no longer fire unconditionally); write `<name>=on`. Checked + present → keep; write `=on`; no action.
6. UNCHECKED resolution: write `<name>=off`. IF absent → suppress the install (this is the opt-out; supersedes newest-wins the 2026-06-12 "absence alone" / "by default, not opt-in" auto-installs for this companion). IF present AND skill-builder-scaffolded → DEFER a `/skill-builder strip <name> --execute` row (add `--confirm-breaking` when HARD references exist) to the Deferred Items table. NEVER an AUTO delete — the user ratified "Always defer to strip", and the destructive-work-DEFERS floor (deletion delegated to strip, never reimplemented) is honored, not superseded. The DEFER note states that strip performs the COMPLETE cross-reference disconnection across every other skill plus the `route index`/embed refresh (why this gate fires SECOND — before the audit commits its terminal route tasks).
7. THE ONE WRITE. This gate's only write is the per-project `<!-- companion-skills: text-eval=…,code-evaluator=…,route=…,awareness-ledger=… -->` marker in references/model-lanes.md (the checkbox answer IS the consent, like the Lane→Model picker). Back-compat: a legacy `<!-- creative-scrub-build: off -->` marker reads as `text-eval=off`. It never switches the session model and never asks one to.
8. IF any change proposes auto-deleting on uncheck, exposing a user-authored skill for removal, removing in a headless run, or removing without routing through `strip` → STOP and report the directive conflict.
<!-- END ENFORCEMENT ANNOTATION -->
