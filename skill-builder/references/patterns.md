...
- **Self-heal is reactive, not ambient** — Self-heal triggers when a user disagrees with the AI about following a directive. It does not run on a schedule, at session end, or in the background. It activates in the moment of the disagreement, diagnoses whether the skill's non-directive wording caused the misinterpretation, and proposes a surgical fix. No disagreement, no trigger. (2026-03-06)

- **Surgical updates require two-agent validation** — Self-heal uses two `context: none` agents in sequence: root-cause-analyst to determine whether the skill's wording caused the directive misinterpretation, then patch-reviewer to validate the proposed fix is minimal, complete, and safe. One agent is not enough — the analyst is motivated to find a cause (it's their job), the reviewer is motivated to reject bad patches (it's their job). Separation of concerns produces better patches. (2026-02-25)

- **The trigger block is always last** — The Self-Heal trigger block must be the final section in any skill's SKILL.md. This ensures it doesn't interfere with the skill's primary workflow instructions. If optimization moves it, move it back. (2026-03-06)
...
- **Eval and self-heal are complementary, not redundant** — Eval measures output quality against a rubric (scored). Self-heal fixes skill wording that causes directive misinterpretation (triggered by user disagreement). They cross-reference: self-heal checks eval-history.md for pending compensations on the same instruction area, eval pre-flight checks self-heal-history.md for recent patches that may affect rubric calibration. Neither replaces the other. (2026-03-06)
