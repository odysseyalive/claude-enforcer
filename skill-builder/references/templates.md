...
6. HARD directives (explicit IDs, concrete never/always rules, measurable conditions) do NOT need annotations — skip them — UNLESS the directive is habit-fighting (it opposes an automatic model tendency: formatting tics, punctuation reflexes, tone habits; see [enforcement.md](enforcement.md) § "Research Grounding"). Habit-fighting directives get an annotation with the Principle Restatement step even when HARD, and are first-order hook candidates.
...
### Principle Restatement step (habit-fighting directives)

For habit-fighting directives (defined in [enforcement.md](enforcement.md) § "Research Grounding"), the CHECKPOINT's first numbered step makes the model articulate the governing principle before executing the gate:

    CHECKPOINT — [gate name]:
    1. State, in one sentence, the principle this checkpoint enforces: "[principle]". Then proceed.
    2. [explicit action]
    ...

Worked example, for the directive "Never use em-dashes in drafted copy":

    CHECKPOINT — Em-Dash Gate:
    1. State the principle: "Em-dashes are a reflexive habit, not a choice; this copy uses commas, periods, or parentheses instead."
    2. Scan the drafted text for the em-dash character.
    3. IF any found → STOP. Rewrite each occurrence with a comma, period, or parentheses. Re-scan.
    4. IF none found → CONTINUE.

Rationale: restating the principle at the decision point loads the relevant concepts into the model's workspace at the moment they matter (the Counterfactual Reflection Training result; [enforcement.md](enforcement.md) § "Research Grounding", finding 3). The step is generated machinery: it sits outside the immutable block and never rewords the source directive.
...
- [enforcement.md](enforcement.md) §§ "Opus 4.7 Behavioral Contract", "Research Grounding"
