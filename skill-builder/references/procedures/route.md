...
2. Lane preflight at the door (2-Brain Harness, 2026-06-06; advisory-only per the No-Switch-Prompt directive, 2026-06-06). BEFORE announcing: read the chosen target's Lane column from the catalog (DECLARED provenance only — an UNCONFIRMED or absent lane never advises; a per-function lane row for the resolved mode wins over the skill-level lane) and read the active model ID from the session system-context line ("The exact model ID is …", strip `[1m]`/`[200k]`, lowercase). IF the lane's preferred model ≠ the active model → print ONE advisory line — "Lane advisory: this work is in the `<lane>` lane (preferred `<preferred>`); session is on `<active>`." — then dispatch. NEVER an AskUserQuestion, NEVER an instruction to run `/model`; `/route` can never switch the model itself and never asks the user to. This single advisory covers the WHOLE endeavor: the dispatched skill's own Model-Lane Preflight sees route's coverage and stays silent, as do skills it chains. Headless / non-interactive → skip this clause silently and dispatch.
...
4. Compare. Match → silent no-op. Mismatch → print ONE advisory line and PROCEED: "Lane advisory: this skill declares the `<lane>` lane (preferred `<preferred>`); the active model is `<active>`." NEVER an AskUserQuestion, NEVER an instruction to run `/model`, never blocking (No-Switch-Prompt directive, 2026-06-06). A skill can NEVER switch the session model and never asks the user to.
