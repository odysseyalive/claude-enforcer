...
**Uncovered-excursion recommendations:** while scanning, IF a lane-declared skill has cross-lane signals (per lane-delegation.md § Excursion Signal Vocabulary, heuristic pass only — no panel here) with no covering map entry → list it as a recommendation in the report. Step 9 itself never designs or inserts. **Under audit (2026-07-22 no-deferrals directive), name no command:** audit's Step 4f.5-bis already queues `agents [skill] --execute` as an AUTO task, so the line reads "excursion uncovered — queued by audit" and the recommendation text is suppressed (audit.md § Audit Autonomy Gate clause 4a). Standalone `route embed` keeps the named recommendation.
