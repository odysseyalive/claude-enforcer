...
- **Protective** and **Load-bearing + not recoverable** → an agent panel determines the correct action (recover vs unwire) based on the skill's current state, file content, and git history. The panel's decision auto-executes. **Under audit (2026-07-22 no-deferrals directive):** these never stop the run — the agent panel resolves, the action executes, and failures land as ✗ items in the Execution Summary. **Standalone:** add a task labeled "USER DECISION REQUIRED — [skill] [hook]" that prints the recommendation; the user re-runs with the decision encoded (e.g., `/skill-builder hooks [skill] --execute --recover=<file>` or `--unwire=<file>`).
