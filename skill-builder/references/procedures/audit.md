...
| 3 | § 0.3 | Companion-Skill checkbox | every interactive audit (not `--quick`/headless) |
...
| Companion | Function (signal-based presence test) | Install task |
|-----------|----------------------------------------|--------------|
| `text-eval` | evaluator-function skill per [creative-integrity.md](../creative-integrity.md) § Build Scaffolds pre-build guards (signal-based, never name-based) | Step 4c-bis evaluator scaffold |
| `code-evaluator` | `.claude/skills/code-evaluator/SKILL.md` exists, OR a skill performing the code-quality-evaluator function | Step 4a-bis `code-eval create` |
| `route` | `.claude/skills/route/SKILL.md` exists | Step 4g `route index` bootstrap |
| `awareness-ledger` | `.claude/skills/awareness-ledger/SKILL.md` exists | Step 4a `ledger --execute` |

(No per-row label column — the "(recommended)" mechanic is superseded for this gate, see below. `route` and `awareness-ledger` remain the install-on-absence defaults, so under the inverted widget they are kept/installed unless explicitly checked for opt-out.)
...
1. **Read the marker.** Read the per-project `<!-- companion-skills: text-eval=…,code-evaluator=…,route=…,awareness-ledger=… -->` marker from `references/model-lanes.md` (§ Companion Skills). Back-compat: a legacy `<!-- creative-scrub-build: off -->` marker reads as `text-eval=off`. A missing marker means "never configured" — the first-run state is the SAFE default under the inverted widget: every box unchecked = keep all present companions and install the absent install-on-absence ones (`route`, `awareness-ledger`, `text-eval`, `code-evaluator`). The marker is written from the user's submission (step 7).
2. **Compute presence (signal-based, never name-based).** For each companion, run its presence test from the table. For `text-eval`, reuse the exact Step 4c-bis / § Build Scaffolds signal test (any § Scope-signal skill counts under any name) so the checkbox agrees with the scaffold step and never offers to install a second evaluator. On-disk presence wins for the displayed PRESENT/ABSENT row state; the marker records intent.
...
4. **Render the checkbox (INVERTED — a check means OPT OUT).** `AskUserQuestion` has no `selected`/`default` field, so a multi-select **cannot pre-check boxes** — every box renders empty regardless of what the procedure "intends." Pre-checking present companions is impossible at the tool layer. To keep the tool's natural empty default SAFE, the gate **inverts** the checkbox meaning: **a checked box opts the companion OUT (remove if present / skip install if absent); an unchecked box keeps or installs it.** Leaving every box unchecked = "keep everything as-is, install the absent install-on-absence companions" — the safe default, confirmed in one submission. Render ONE multi-select AskUserQuestion, one row per companion, with each row stating its **PRESENT/ABSENT** state in the label/description (e.g. `text-eval — PRESENT (check to remove)`, `code-evaluator — ABSENT (check to skip install)`). **No "(recommended)" wording appears** (superseded — see the note above). This is a CONFIGURATION question — its answer IS consent (like the Lane→Model picker), not a switch prompt. (Inversion ratified 2026-06-24 to reconcile the directive's "checkmarks by present items" intent with the tool's inability to pre-select: present companions are retained BY DEFAULT, which is the intent; the visible mechanism is opt-out rather than a rendered checkmark.)
5. **Resolve UNCHECKED = KEEP (the default path).** Unchecked + present → keep; mark `=on`; no action. Unchecked + absent → AUTHORIZE the companion's install task (this gate is now the authorization for the Step 4a / 4a-bis / 4c-bis / 4g appends — they no longer fire unconditionally; see those steps); mark `=on`.
6. **Resolve CHECKED = OPT OUT.** Mark `=off`.
   - Absent → suppress the install (the opt-out; supersedes newest-wins the 2026-06-12 "absence alone" / "by default, not opt-in" auto-install for this companion). Nothing to remove.
...
Bootstrap is a full interactive audit, so the Step 0.3 companion gate runs (right after the disclaimer) and GATES the companion installs below — a fresh project is exactly where the install opt-in matters. Under the inverted widget the companion installs are authorized unless their box is checked for opt-out (unchecked = install; `route`, `awareness-ledger`, `text-eval`, and `code-evaluator` install on absence by default — the empty default keeps them all).
...
| Companion-gate removals (Step 0.3 — a PRESENT, skill-builder-scaffolded companion was UNCHECKED) | `/skill-builder strip <companion> --execute [--confirm-breaking]` — the user ratified "Always defer to strip"; the checkbox suppresses re-install and defers removal, never an AUTO delete. Strip performs the COMPLETE cross-reference disconnection across every other skill plus the `route index`/embed refresh. User-authored (non-scaffolded) companions are never offered for removal (provenance guard) |
