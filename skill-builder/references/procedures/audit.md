...
4. This gate asks about the disclaimer ONLY. It never asks the user to switch models — no audit step does (No-Switch-Prompt Gate). The full audit asks at most FOUR questions ever (count raised from three by the 2026-06-24 directive): this disclaimer, the Step 0.3 Companion-Skill Selection Gate, the one-time 4f-setup onboarding, and the every-audit Lane→Model picker (Audit Autonomy Gate clause 1).

### Step 0.3: Companion-Skill Selection Gate (fires SECOND — right after Step 0, before Step 0.5)

Per the 2026-06-24 sacred directive (SKILL.md § Directives → Companion-Skill Selection Gate), the audit's SECOND question is a multi-select checkbox that lets the user opt in/out of the evaluator/helper companion skills audit force-installs. It fires immediately after the Step 0 disclaimer is accepted and BEFORE the Step 0.5 VCS preflight, so install/remove intent is known before the audit builds its execution plan and its terminal route tasks (Step 4g) — the user's stated reason for the second-in-line placement ("the route index and route embedding will affect the outcome too").
