...
5. THE GATE NEVER REMOVES (2026-07-17 — supersedes newest-wins the 2026-06-24 "If an item is unchecked, then audit should remove that skill set from the project" clause and the inverted checked-to-remove mechanic; the immutable text stands byte-verbatim, superseded in prose only). No checkbox state maps to removal and no DEFER `strip` row originates from this gate. Uninstall is exclusively the user-run `/skill-builder strip <name> --execute` — strip remains the ONLY deletion path, so "Always defer to strip" and the destructive-work-DEFERS floor are honored by making strip the whole removal story rather than feeding it from a checkbox.
...
1. **The single onboarding question (2-Brain Harness directive, 2026-06-06: "at the audit's single onboarding question, the user picks the creative model and the model for everything else").** Emit ONE batched **AskUserQuestion** call containing FOUR question objects — never separate prompts (this reaches AskUserQuestion's four-object cap; any future onboarding question forces a redesign):
   ...
   - *(d) header "Advisor model (global)":* the global-advisor question per [lane-delegation.md](../lane-delegation.md) § Global Advisor Model — FULL official model IDs drawn from the SAME pool as (b)/(c) (the Lane→Model Picker statics + discovered latest; `claude-fable-5` always offered, never dropped), pairing-filtered against the detected main model, plus "No advisor"; omitted when the `advisor-setup` marker is `declined`.
...
*(Companion removal is never audit work — not AUTO, not DEFER. Per the 2026-07-17 install-only directive, the Step 0.3 gate only installs; uninstalling a companion is exclusively the user's own `/skill-builder strip <name> --execute` run, whenever they choose.)*
