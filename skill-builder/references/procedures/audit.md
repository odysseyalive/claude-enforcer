...
1. **Interactive session** → present via AskUserQuestion and STOP until answered:
   ...
   > - A backup process is available — right after this you'll be asked whether to snapshot your CLAUDE.md and .claude directory (kept under .claude-backups/, the last 3 plus a permanent first-run baseline). Taking it lets you cleanly uninstall claude-enforcer later and restore your original setup with `/skill-builder restore`.

   Options EXACTLY: **Accept and proceed** / **Cancel audit**. On **Cancel audit** → STOP entirely; report that nothing was scanned or written. (The first three bullets are sacred minimum content, verbatim; the fourth and fifth are machinery disclosure for informed consent — the fifth announces the Step 0.2 backup offer.)
...
| Companion | Function (signal-based presence test) | Install task | Label |
|-----------|----------------------------------------|--------------|-------|
...
| `route` | `.claude/skills/route/SKILL.md` exists | Step 4g `route index` bootstrap | **(recommended)** |
| `awareness-ledger` | `.claude/skills/awareness-ledger/SKILL.md` exists | Step 4a `ledger --execute` | **(recommended)** |

`route` and `awareness-ledger` carry the **(recommended)** label in their checkbox row and are pre-checked when present — per the user directive ("in parentheses next to it it shows as recommended item").

**Procedure:**

1. **Read the marker.** Read the per-project `<!-- companion-skills: text-eval=…,code-evaluator=…,route=…,awareness-ledger=… -->` marker from `references/model-lanes.md` (§ Companion Skills). Back-compat: a legacy `<!-- creative-scrub-build: off -->` marker reads as `text-eval=off`. A missing marker means "never configured" — first-run defaults are computed from on-disk presence (present → checked; the two recommended companions default checked even when absent, signaling the recommendation).
2. **Compute presence (signal-based, never name-based).** For each companion, run its presence test from the table. For `text-eval`, reuse the exact Step 4c-bis / § Build Scaffolds signal test (any § Scope-signal skill counts under any name) so the checkbox agrees with the scaffold step and never offers to install a second evaluator. On-disk presence wins for the pre-check; the marker records intent.
...
4. **Render the checkbox.** ONE multi-select AskUserQuestion, one row per companion, present → pre-checked, recommended pair labeled `(recommended)`. This is a CONFIGURATION question — its answer IS consent (like the Lane→Model picker), not a switch prompt. Confirming the pre-checked state is one submission.
5. **Resolve CHECKED.** Checked + present → keep; mark `=on`; no action. Checked + absent → AUTHORIZE the companion's install task (this gate is now the authorization for the Step 4a / 4a-bis / 4c-bis / 4g appends — they no longer fire unconditionally; see those steps); mark `=on`.
6. **Resolve UNCHECKED.** Mark `=off`.
...
8. **Feed the plan.** Authorized installs flow into the Step 6 AUTO tier exactly where Steps 4a / 4a-bis / 4c-bis / 4g already place them (the gate only decides WHETHER they are appended). DEFER `strip` rows flow into the Deferred Items table. Because this ran SECOND, the terminal `route index` / `route embed` tasks (Step 4g) reflect the install choices in the same run; removals run later via the deferred `strip` command, which itself refreshes the index/embed.

**Absence-vs-gap note.** The gate is a STANDING question (it fires every full interactive audit, like the disclaimer and picker — it is the user's standing install/removal control), so it is not omitted merely because every box is pre-checked. It is suppressed ONLY by the headless/`--quick` rule above.
...
Bootstrap is a full interactive audit, so the Step 0.3 companion gate runs (right after the disclaimer) and GATES the companion installs below — a fresh project is exactly where the install opt-in matters. The companion rows are appended only when their box is checked (the two recommended companions, `route` and `awareness-ledger`, default checked).
...
**Companion-gate interaction (Step 0.3, 2026-06-24).** `route` carries the **(recommended)** label in the Step 0.3 checkbox and is pre-checked. The terminal `route index`/`route embed` tasks run normally whenever `route` is checked (or absent-and-checked → bootstrap), which is the overwhelmingly common case. Two edge resolutions: `route` unchecked + ABSENT → suppress the bootstrap (do not append `route index`/`route embed`; the project has opted out of the dispatcher); `route` unchecked + PRESENT → still run `route index`/`route embed` THIS audit (so the catalog reflects the run's other changes consistently), and surface the removal as a DEFER `/skill-builder strip route --execute --confirm-breaking` row (strip is almost always BREAKING for `route` since the four managed-block families reference it) — never an AUTO delete. Removing the dispatcher is high-blast-radius, which is why it is recommended-and-deferred, never automatic.
